...
function applyV42Dashboard() {
  ...
  if (!document.getElementById('dashboard-v42-style')) {
    const style = document.createElement('style');
    style.id = 'dashboard-v42-style';
    style.textContent = `
      ...
      .v42-info-grid{display:grid;grid-template-columns:repeat(4,1fr);border-top:1px solid #2d3e50;border-bottom:1px solid #2d3e50;margin:24px 0 18px;padding:20px 0;}
      .v42-info{padding:0 10px;text-align:center;border-inline-start:1px solid #2d3e50;}
      .v42-info:first-child{border-inline-start:none;}
      .v42-info .ico{font-size:22px;display:block;margin-bottom:7px;}
      .v42-info .lbl{font-size:13px;color:#9aa6b4;}
      .v42-info .val{font-size:18px;font-weight:800;margin-top:2px;white-space:nowrap;}
      .v42-info .sub{font-size:12px;color:#7f8d9d;margin-top:2px;}
      .v42-info.pay .val{color:#62dd7a;}

      .v42-tag-title{font-size:15px;font-weight:700;color:#98a5b4;margin:10px 0;}
      .v42-tags{display:flex;gap:9px;justify-content:center;flex-wrap:wrap;margin-bottom:18px;}
      .v42-chip{border-radius:999px;padding:7px 13px;border:1px solid #3f5872;background:#14263a;color:#d9e7f7;font-size:13px;font-weight:700;}
      .v42-chip.orange{border-color:#7f5617;background:#2a2111;color:#efb34a;}
      .v42-chip.purple{border-color:#67427d;background:#24192d;color:#d3a0ec;}

      .v42-actions{display:grid!important;grid-template-columns:1fr 1fr;gap:12px!important;margin-top:18px!important;}
      .v42-actions .clock-btn,.v42-actions #cancelShiftBtn{min-height:92px!important;border-radius:22px!important;font-size:23px!important;font-weight:900!important;display:flex!important;align-items:center!important;justify-content:center!important;margin:0!important;}
      .v42-actions .clock-btn.stop{background:#111923!important;border:2px solid #57d975!important;color:white!important;}
      .v42-actions #cancelShiftBtn{background:#111923!important;border:2px solid #ff5858!important;color:white!important;}
      ...
      .v42-bottom-nav{position:fixed;left:50%;transform:translateX(-50%);bottom:0;width:min(900px,100%);z-index:9999;display:grid;grid-template-columns:repeat(4,1fr);padding:11px 14px 13px;background:rgba(12,18,26,.97);backdrop-filter:blur(16px);border:1px solid #283747;border-bottom:none;border-radius:22px 22px 0 0;box-shadow:0 -10px 28px rgba(0,0,0,.25);}
      .v42-nav{text-align:center;color:#8f9baa;font-size:12px;}
      .v42-nav .ico{display:block;font-size:22px;line-height:1;margin-bottom:5px;}
      .v42-nav.active{color:#61dc79;font-weight:800;}

      @media(max-width:640px){
        header.top h1{font-size:26px!important;}
        .v42-info-grid{grid-template-columns:repeat(2,1fr);row-gap:18px;}
        .v42-info:nth-child(3){border-inline-start:none;}
        .v42-actions .clock-btn,.v42-actions #cancelShiftBtn{min-height:86px!important;font-size:21px!important;}
      }
    `;
  ...
  if (card && !card.querySelector('.v42-info-grid')) {
    const timerSub = document.getElementById('timerSub');
    const grid = document.createElement('div');
    grid.className = 'v42-info-grid';
    grid.innerHTML = `
      <div class="v42-info"><span class="ico">📅</span><div class="lbl">תאריך</div><div class="val" id="v42Date">—</div><div class="sub" id="v42Day">—</div></div>
      <div class="v42-info"><span class="ico">🕒</span><div class="lbl">שעות</div><div class="val" id="v42Hours">—</div><div class="sub">זמן אמת</div></div>
      <div class="v42-info pay"><span class="ico">💵</span><div class="lbl">שכר משוער</div><div class="val" id="v42Pay">—</div><div class="sub">עד כה</div></div>
      <div class="v42-info"><span class="ico">⏱️</span><div class="lbl">משך</div><div class="val" id="v42Duration">—</div><div class="sub">שעות</div></div>`;
    if (timerSub) timerSub.insertAdjacentElement('afterend', grid);

    const title = document.createElement('div');
    title.className = 'v42-tag-title';
  ...
  if (!document.querySelector('.v42-bottom-nav')) {
    ...
    nav.innerHTML = '<div class="v42-nav active"><span class="ico">⌂</span>בית</div><div class="v42-nav"><span class="ico">◷</span>היסטוריה</div><div class="v42-nav"><span class="ico">▥</span>דיווחים</div><div class="v42-nav"><span class="ico">⚙</span>הגדרות</div>';
  ...
  const refresh = () => {
    ...
    if (!startMs) {
      set('v42Date', '—'); set('v42Day', '—'); set('v42Hours', '—'); set('v42Pay', '₪0.00'); set('v42Duration', '0:00');
      return;
    }
